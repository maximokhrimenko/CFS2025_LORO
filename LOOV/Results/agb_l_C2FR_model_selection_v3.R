...
rmse_threshold <- baseline_rmse + 3.0
bias_threshold <- baseline_bias + 3.0
regional_rmse_threshold <- baseline_regional_rmse + 3.0
regional_bias_threshold <- baseline_regional_bias + 3.0

### Step 4: Save baseline and threshold values to a text file
threshold_file <- "selected_models_agb_l_C2FR_baselines_and_thresholds.txt"
capture.output({
...
selected_models <- df.agb_l %>%
  filter(
    ...
    Metrics_input == "C2FR" # Additional condition
  )
...
filtered_models_file <- "selected_models_agb_l_C2FR.csv"
