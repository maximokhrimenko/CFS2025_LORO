...
for (i in 1:nrow(models.var.cv)) {
  ...
  if (model_form == "loglog") {
    ...
    if (!"log_agb_lad" %in% colnames(df_filtered)) {
      df_filtered <- df_filtered %>% mutate(log_agb_lad = log(agb_lad))
    }
    
    # Create the formula using log-transformed variables
    log_vars <- paste0(vars, "_log")  # Use the new transformed column names
    formula <- as.formula(paste("log_agb_lad ~", paste(log_vars, collapse = " + ")))
  } 
  ...
  if (model_form == "linear") {
    ...
    df_filtered <- df_filtered %>%
      mutate(
        ...
        residuals = agb_lad - predictions   # Residuals on back-transformed scale
      )
  }
