...
# Loop through 1 to 10 components for PCR
...
for (n_comp in 1:max_components) {
  ...
  for (fold in folds) {
    ...
    # Fit the PCR model with the specified number of components
    pcr_model <- pcr(agb_l ~ ., data = train_data, ncomp = n_comp, validation = "none")
    
    # Make predictions on the test set
    predictions <- predict(pcr_model, newdata = test_data[, predictors], ncomp = n_comp) %>% as.vector()
...
# Fit the final PCR model with the optimal number of components
final_pcr_model <- pcr(agb_l ~ ., data = df_filtered[, c("agb_l", predictors)], 
                       ncomp = optimal_components, validation = "none")

# Fit the single-component full model
full_model_1comp <- pcr(agb_l ~ ., data = df_filtered[, c("agb_l", predictors)], 
...
summary_file <- "PCR_Model_Summary_agb_l.txt"
capture.output(
  {
...
df_filtered <- df_filtered %>%
  mutate(predictions_optimal = predict(final_pcr_model, newdata = df_filtered[, predictors], 
...
plot_optimal <- ggplot(df_filtered, aes(x = agb_l, y = predictions_optimal, color = LOCATION)) +
  ...
  labs(
    title = paste0(
      "PCR Regression: Predicted vs Observed (Optimal: ", optimal_components, " Components)\n",
...
ggsave(filename = "PCR_Model_Predicted_vs_Observed_Optimal_agb_l.png", plot = plot_optimal, width = 8, height = 6)
...
plot_1comp <- ggplot(df_filtered, aes(x = agb_l, y = predictions_1comp, color = LOCATION)) +
  ...
  labs(
    title = paste0(
      "PCR Regression: Predicted vs Observed (1 Component)\n",
...
ggsave(filename = "PCR_Model_Predicted_vs_Observed_1Component_agb_l.png", plot = plot_1comp, width = 8, height = 6)

# Save cross-validation results
write.csv(cv_results, "PCR_CrossValidation_Results_agb_l.csv", row.names = FALSE)

print("PCR modeling completed. Results and plots saved.")
