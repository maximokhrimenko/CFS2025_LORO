...
df.agb_lad <- read.csv("df.agb_lad.csv")

### Step 2: Find the baseline values (min values for criteria)
baseline_rmse <- min(df.agb_lad$n.rmse.LORO, na.rm = TRUE)
baseline_bias <- min(df.agb_lad$n.bias.magnitude.LORO, na.rm = TRUE)
baseline_regional_rmse <- min(df.agb_lad$n.rmse.regional.max, na.rm = TRUE)
baseline_regional_bias <- min(df.agb_lad$n.bias.regional.max, na.rm = TRUE)
...
threshold_file <- "selected_models_agb_lad_baselines_and_thresholds.txt"
capture.output({
...
filtered_models_file <- "selected_models_agb_lad.csv"
