...
### in agb_lad all models selected are linear, thus no loglog trouble
rm(list = ls())
...
models.var.cv <- read.csv("selected_models_agb_lad.csv") # Contains x1, x2, x3 columns for variable combinations

# Create a folder to save plots and summaries if it doesn't exist
if (!dir.exists("models_lad")) {
  dir.create("models_lad")
}
...
for (i in 1:nrow(models.var.cv)) {
  ...
  formula <- as.formula(paste("agb_lad ~", paste(vars, collapse = " + ")))
  ...
  df_filtered <- df_filtered %>%
    mutate(residuals = agb_lad - predict(model))
  ...
  summary_file_path <- file.path("models_lad", paste0("Model_", i, ".txt"))
  capture.output(
    {
      cat("Model Summary:\n")
      print(summary_model)
  ...
  plot <- ggplot(df_filtered, aes(x = agb_lad, y = predictions, color = LOCATION)) +
  ...
  ggsave(filename = paste0("models_lad/model_", i, ".png"), plot = plot, width = 8, height = 6)
}
