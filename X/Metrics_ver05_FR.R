...
# Define years and base directory
years <- c(2016, 2018, 2019, 2021)
base_dir <- "C:/X" # Update this to your base directory containing year folders

# Create an empty data frame to store combined results
combined.df <- main.df.empty


# Outer loop for each year
for (year in years) {
  setwd(file.path(base_dir, as.character(year))) # Change directory to the year's folder
  lidar.year <- year
  
  # Load the list of files for the current year
  files.list.Step6 <- list.files(path = "Step6")
  
  # Initialize a temporary data frame for the current year
  main.df <- main.df.empty
...
# Combine yearly data with the final data frame
  combined.df <- rbind(combined.df, main.df)
}


write.csv(combined.df, "combined.df.FR.csv", row.names=FALSE)
